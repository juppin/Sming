...
static DEFINE_FSTR(ATTR_SUNRISE, "Sunrise");
static DEFINE_FSTR(ATTR_SUNSET, "Sunset");
...
void CSystemCommands::handleMessage(command_connection_t connection, JsonObject& json)
{
	...
	if(COMMAND_INFO == command) {
		...
		json[ATTR_SUNRISE] = timeManager.timeStr(time_sunrise, 0);
		json[ATTR_SUNSET] = timeManager.timeStr(time_sunset, 0);
